...
import com.src.myjspProject.repository.DashBoradRepository;
...
public class RepositoryService {
     ...
     */
    @Transactional
    public String getDashBoradData( DashBoardeDataDTO valueData) throws Exception {
    	SimpleDateFormat date = new SimpleDateFormat("yyyy-MM-dd:HH:mm");
        Date times = new Date();
        valueData.setTime(date.format(times));
    	DashBoardeData data = dashBoradRepository.save(valueData.toEntity());

    	return "dashborad/dashborad";
    }

    @Transactional
    public List<DashBoardeData> getDashBoradDataOut(DashBoardeData dashBoardeData) throws Exception{


    	List<DashBoardeData> data = dashBoradRepository.findAll();
